...
Supported target types: `path`, `path_prefix`, `path_info_prefix`, `path_regex`, `node`/`content_node`, `subtree`/`ibexa_subtree`. Supported condition types: `siteaccess` and `content_type`/`ibexa_content_type` (unknown condition types match by default). The `ibexa_*` aliases keep imported Netgen Layouts transfer files working.
