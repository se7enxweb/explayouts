...
class expLayoutsRelationListQueryHandler
{
    public static function getParameters()
    {
        return array(
            ...
            'sort_type' => array( 'type' => 'select', 'options' => array( 'defined_by_field', 'date_published', 'date_modified', 'content_name' ), 'default' => 'defined_by_field' ),
    ...
    public static function getValues( $params, $offset = 0, $limit = null )
    {
        ...
        foreach ( $related as $relatedObject )
        {
            ...
            if ( !empty( $params['only_main_locations'] ) )
                $node = $relatedObject->mainNode();
            else
            {
                $nodeList = eZContentObjectTreeNode::fetchByContentObjectID( (int)$relatedObject->attribute( 'id' ) );
                $node = is_array( $nodeList ) && count( $nodeList ) > 0 ? $nodeList[0] : false;
            }
    ...
    {
        $sortType = isset( $params['sort_type'] ) ? $params['sort_type'] : 'defined_by_field';
